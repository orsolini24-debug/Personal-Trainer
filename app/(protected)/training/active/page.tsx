  ...
  const trackerData = {
    ...
    exercises: exercises.map(ex => {
      ...
      return {
        ...
        sets: ex.sets ?? 3,
        repsMin: ex.repsMin ?? parsedNotes.repsMin ?? parseInt(ex.reps?.split('-')[0] ?? '8'),
        repsMax: ex.repsMax ?? parsedNotes.repsMax ?? parseInt(ex.reps?.split('-').pop() ?? '12'),
        targetRir: ex.rir ?? 2,
        restSec: ex.restSec ?? parsedNotes.restSec ?? 120,
        planNotes: ex.planNotes ?? parsedNotes.plan ?? ex.technicalNotes ?? '',
