  ...
  return (
    <div className="space-y-8 py-6">
      <div className="text-center max-w-2xl mx-auto space-y-2">
        <h2 className="text-3xl font-black text-[#f1f5f9] tracking-tight">Le tue Strategie AI</h2>
        <p className="text-[#64748b]">Ho analizzato il tuo profilo. Ecco 3 percorsi distinti per il prossimo mesociclo.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {proposals.map((p) => (
          <div 
            ...
            className={`relative p-6 rounded-[2.5rem] border transition-all duration-500 cursor-pointer group flex flex-col h-full ${selected === p.id ? 'bg-[#3b82f6]/5 border-[#3b82f6] shadow-[0_0_40px_rgba(59,130,246,0.15)]' : 'bg-[#111118] border-white/5 hover:border-white/20'}`}
          >
            {p.isRecommended && (
            ...
            <div className="mb-6">
              <h3 className={`text-xl font-black mb-2 transition-colors ${selected === p.id ? 'text-[#3b82f6]' : 'text-[#f1f5f9]'}`}>{p.name}</h3>
              <p className="text-sm text-[#64748b] leading-relaxed line-clamp-3 group-hover:line-clamp-none transition-all">{p.strategy}</p>
            </div>
            ...
            <div className="space-y-4 flex-1">
              <div className="space-y-2">
                ...
                {p.pros.map((pro, i) => (
                  <div key={i} className="flex items-start gap-2 text-xs text-[#f1f5f9]/80">
            ...
            <div className={`mt-8 w-full py-3 rounded-2xl flex items-center justify-center font-bold text-sm transition-all ${selected === p.id ? 'bg-[#3b82f6] text-white' : 'bg-white/5 text-[#64748b]'}`}>
